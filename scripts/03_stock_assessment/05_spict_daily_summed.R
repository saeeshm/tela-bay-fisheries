...
# Description: Performing a stock-assessment with the surplus production in
# continuous time (SPiCT) model, based on total catches and total effort per day.
...
# Summing total catch per trip (since for any given trip, defined by the
# codigo, each individual fish is recorded separately, but the hours of effort
# is the same)
fdb <- fdb_full |> 
  group_by(codigo) |>
  summarize(
    ...
    horas_pesca = unique(horas_pesca),
...
inps <- map(inps_raw, \(inp){
  ...
  # Prior uncertainties of certain years of data
  inp$stdevfacC <- rep(1, length(inp$obsC))
  # inp$stdevfacC[1:2] <- 2
  inp$stdevfacC[3:6] <- 0.8
  inp$stdevfacC[14:15] <- 5
  inp$stdevfacE <- rep(1, length(inp$obsE))
  inp$stdevfacE[3:6] <- 0.8
  inp$stdevfacE[14:15] <- 5
  # Prior guess of biomass level relative to carrying capacity before start of
  # data
  inp$priors$logbkfrac <- c(log(0.2),2,1)
...
my_plotspict.catch(resnull$wk)
my_plotspict.catch(res$day)
my_plotspict.catch(res$wk)
my_plotspict.catch(res_sde$day)
my_plotspict.catch(res_sde$wk)
my_plotspict.catch(res_sde_rob$wk)
