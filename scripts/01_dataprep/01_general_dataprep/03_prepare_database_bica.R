...
path_roatan <- 'data/raw/bica//Desembarco Roatan.xlsx'
path_guanaja <- 'data/raw/bica/Desembarcos Guanaja.xlsx'
path_utila <- 'data/raw/bica/Desembarcos Utila.xlsx'
...
# Creating an improved ID column
id_index <- setNames(c('GNJ', 'ROA', 'UTL'), c('Guanaja', 'Roatan', 'Utila'))
master <- master %>%
  mutate(uid = 1:nrow(.), .before=id) |> 
  mutate(uid = paste0(id_index[zona], str_pad(uid, 3, 'left', '0')))

# Extra cleaning of species names (added-later) ----------

# Inverting fields for some columns where the scientific names contain the common names
master <- master |>
  mutate(nombre_comun = if_else(
    str_detect(tolower(nombre_cientifico), '(C\\s|rojo|red|mutton|jimmy|snaper|hint|snapper)'),
    nombre_cientifico,
    nombre_comun
  )) |> 
  mutate(nombre_cientifico = if_else(
    str_detect(tolower(nombre_cientifico), '(C\\s|rojo|red|mutton|jimmy|snaper|hint|snapper)'),
    NA_character_,
    nombre_cientifico
  ))
...
other_names <- names(master)[!names(master) %in% colidx]
master <- master |> select(all_of(c(colidx, other_names)))

# Writing the full timeseries to disk
write_csv(master, file.path(out_dir, 'bica_tidied.csv'))
