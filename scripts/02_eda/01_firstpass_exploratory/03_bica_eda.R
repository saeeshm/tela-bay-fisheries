...
ts_path <- 'data/bica_master_dbase_clean.csv'
...
dat <- dat |> 
  ...
  mutate(comunidad = tolower(str_remove_all(comunidad, '\\s+'))) |> 
...
dat |> 
  ...
  ggplot(aes(x = month, y=cpue, colour=month)) +
  geom_boxplot(alpha=0.8, show.legend = F) +
  geom_jitter(alpha=0.3, width=0.2, show.legend=F) +
...
dat |> 
  ...
  summarize(cpue = mean(peso/horas_pesca, na.rm=T)) %>%
  filter(!is.nan(cpue), !is.infinite(cpue)) |> 
  # View()
  # Removing outliers 
  filter(iqr(cpue) == 'No') |>  
  # Further summarizing these by year - using a median
  ...
  # geom_col(colour='black', alpha=0.8, fill='skyblue2') +
  qtheme() +
  facet_wrap('comunidad', nrow=4, ncol=3, scales='free_y') +
  labs(
    x=NULL,
    y='Capturas por unidad de esfuerzo [g/hora]'
  )
