---
title: "Tela Bay Fisheries Summary"
output: 
  html_document:
    toc: true
    toc_depth: 3 
...
# Path to TS dataset
ts_path <- file.path(phome, 'data/dbase_tela_clean.csv')

# Path to output directory where plots will be saved if exporting
out_dir <- file.path(phome, 'output/eda/tela')
...
top10_idx <- top10$nombre_cientifico
...
```{r genr-top10-wt-by-year, fig.height=7}
dat |> 
  filter(nombre_cientifico %in% top10_idx) |> 
  group_by(nombre_comun, year) |> 
  summarize('total_weight_kg' = sum(peso, na.rm=T)) |> 
  ggplot(aes(x = year, y=total_weight_kg, fill=year)) +
  geom_col(colour='black', linewidth=0.4, show.legend = F, alpha=0.9) +
  qtheme() +
...
```{r genr-monitor-effort}
dat |> 
  group_by(year, tipo_arte) |> 
  summarize('peso_muest' = sum(peso, na.rm=T)) |> 
