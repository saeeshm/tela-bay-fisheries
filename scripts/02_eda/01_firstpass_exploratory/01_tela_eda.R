...
ts_path <- 'data/dbase_tela_clean.csv'

# Path to output directory where plots will be stored
out_dir <- 'output/eda/tela'
dir.create(out_dir)
...
# Preparing columns
dat <- dat |> 
  tidyr::unite(nombre_cientifico, genus_lmp, species_lmp, sep=' ', remove=F, na.rm=T) |> 
...
dat <- dat |> 
  group_by(genus_lmp, tipo_longitud) |> 
